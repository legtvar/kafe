    ...
    return (
        <AwaitAPI request={(api) => api.playlists.getById(id)} error={<Status statusCode={404} embeded />}>
            {(playlist: Playlist) => {
                ...
                return (
                    <AwaitMultiAPI request={(api) => artifacts.map((artifact) => api.artifacts.getById(artifact.id))}>
                        {(artifacts) => (
                            <AwaitMultiAPI
                                request={(api) =>
                                    artifacts.map(async (artifact) => {
                                        const videoShard = artifact.shards.filter((shard) => shard.kind === 'video')[0];
                                        if (videoShard) {
                                                ...
                                                <Box width="full">
                                                    {artifacts.length === 0 ? (
                                                        ...
                                                        <ContentViewer
                                                            ...
                                                            onPrevious={
                                                                item > 1 ? () => navigate(`./${item - 1}`) : undefined
                                                            }
                                                        />
                                                    )}
                                                </Box>
                                                            ...
                                                            artifacts.map((artifact, i) => (
                                                                <Link to={`./${i + 1}`} key={i}>
                                                                    <Flex
                                                                        ...
                                                                        cursor="pointer"
                                                                        background={
                                                                            item === i + 1 ? selectedColor : undefined
                                                                        }
                                                                        _hover={{
